...
| Chat Surface (WP#7) | Text chat panel (persisted) with optional `!notes` helper; integrates with backend for MVP | REST router `server/agents/chat.py` + SQLModel table persist session messages; React chat component scopes to the active session, fetches history, posts via `/chat`, adds `!notes` stub entries, and displays inline roll results. | ⚠️ Partial | Decide whether MVP requires a “real” `!notes` summary or keep stub; either way ensure chat is reliable and live updates behave correctly. |
